...
public class MypageController {
    ...
    private void onUpdateClicked() {
        ...
        if (!newPwd.isEmpty() || !confirmPwd.isEmpty() || !currentPwd.isEmpty()) {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/new_schema", "root", "1234");
                 PreparedStatement checkPwdStmt = conn.prepareStatement("SELECT password FROM users WHERE user_ID = ?");
                 PreparedStatement updateStmt = conn.prepareStatement("UPDATE users SET user_phone = ?, profile_image = ?, password = ? WHERE user_ID = ?")) {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/new_schema", "root", "1234");
                 PreparedStatement updateStmt = conn.prepareStatement("UPDATE users SET user_phone = ?, profile_image = ? WHERE user_ID = ?")) {
